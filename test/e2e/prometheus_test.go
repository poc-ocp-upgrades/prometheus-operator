...
func testPromResourceUpdate(t *testing.T) {
	...
	name := "test"
	p := framework.MakeBasicPrometheus(ns, name, name, 1)
	p.Spec.Resources = v1.ResourceRequirements{Requests: v1.ResourceList{v1.ResourceMemory: resource.MustParse("100Mi")}}
	p, err := framework.CreatePrometheusAndWaitUntilReady(ns, p)
	if err != nil {
		t.Fatal(err)
	}
	...
	p.Spec.Resources = v1.ResourceRequirements{Requests: v1.ResourceList{v1.ResourceMemory: resource.MustParse("200Mi")}}
...
`
	cfg := &v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: fmt.Sprintf("prometheus-%s", name)}, Data: map[string][]byte{"prometheus.yaml": []byte(firstConfig), "configmaps.json": []byte("{}")}}
...
`
	secret := v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "additional-scrape-configs"}, Data: map[string][]byte{"prometheus-additional.yaml": []byte(additionalConfig)}}
	_, err := framework.KubeClient.CoreV1().Secrets(ns).Create(&secret)
	if err != nil {
		t.Fatal(err)
	}
	p := framework.MakeBasicPrometheus(ns, prometheusName, group, 1)
	p.Spec.AdditionalScrapeConfigs = &v1.SecretKeySelector{LocalObjectReference: v1.LocalObjectReference{Name: "additional-scrape-configs"}, Key: "prometheus-additional.yaml"}
	if _, err := framework.CreatePrometheusAndWaitUntilReady(ns, p); err != nil {
		t.Fatal(err)
	}
	if finalizerFn, err := testFramework.CreateServiceAndWaitUntilReady(framework.KubeClient, ns, svc); err != nil {
...
`
	secret := v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "additional-alert-configs"}, Data: map[string][]byte{"prometheus-additional.yaml": []byte(additionalConfig)}}
	_, err := framework.KubeClient.CoreV1().Secrets(ns).Create(&secret)
	if err != nil {
		t.Fatal(err)
	}
	p := framework.MakeBasicPrometheus(ns, prometheusName, group, 1)
	p.Spec.AdditionalAlertManagerConfigs = &v1.SecretKeySelector{LocalObjectReference: v1.LocalObjectReference{Name: "additional-alert-configs"}, Key: "prometheus-additional.yaml"}
	if _, err := framework.CreatePrometheusAndWaitUntilReady(ns, p); err != nil {
		t.Fatal(err)
	}
	if finalizerFn, err := testFramework.CreateServiceAndWaitUntilReady(framework.KubeClient, ns, svc); err != nil {
...
func testPromReloadRules(t *testing.T) {
	...
	ruleFile.Spec.Groups = []monitoringv1.RuleGroup{{Name: "my-alerting-group", Rules: []monitoringv1.Rule{{Alert: secondAlertName, Expr: intstr.FromString("vector(1)")}}}}
...
func testPromMultiplePrometheusRulesDifferentNS(t *testing.T) {
	...
	p.Spec.EvaluationInterval = "1s"
	p.Spec.RuleNamespaceSelector = &metav1.LabelSelector{MatchLabels: ruleFilesNamespaceSelector}
...
func generateHugePrometheusRule(ns, identifier string) *monitoringv1.PrometheusRule {
	...
	for i := 0; i < 12000; i++ {
		groups[0].Rules = append(groups[0].Rules, monitoringv1.Rule{Alert: alertName + "-" + identifier, Expr: intstr.FromString("vector(1)")})
	}
...
func testPromOnlyUpdatedOnRelevantChanges(t *testing.T) {
	...
	type versionedResource interface{ GetResourceVersion() string }
	resourceDefinitions := []struct {
		Name			string
		Getter			func(prometheusName string) (versionedResource, error)
		Versions		map[string]interface{}
		MaxExpectedChanges	int
	}{{Name: "prometheus", Getter: func(prometheusName string) (versionedResource, error) {
		return framework.MonClientV1.Prometheuses(ns).Get(prometheusName, metav1.GetOptions{})
	}, MaxExpectedChanges: 1}, {Name: "rulesConfigMap", Getter: func(prometheusName string) (versionedResource, error) {
		return framework.KubeClient.CoreV1().ConfigMaps(ns).Get("prometheus-"+prometheusName+"-rulefiles-0", metav1.GetOptions{})
	}, MaxExpectedChanges: 2}, {Name: "configurationSecret", Getter: func(prometheusName string) (versionedResource, error) {
		return framework.KubeClient.CoreV1().Secrets(ns).Get("prometheus-"+prometheusName, metav1.GetOptions{})
	}, MaxExpectedChanges: 2}, {Name: "statefulset", Getter: func(prometheusName string) (versionedResource, error) {
		return framework.KubeClient.AppsV1().StatefulSets(ns).Get("prometheus-"+prometheusName, metav1.GetOptions{})
	}, MaxExpectedChanges: 3}, {Name: "service-operated", Getter: func(prometheusName string) (versionedResource, error) {
		return framework.KubeClient.CoreV1().Services(ns).Get("prometheus-operated", metav1.GetOptions{})
	}, MaxExpectedChanges: 1}, {Name: "serviceMonitor", Getter: func(prometheusName string) (versionedResource, error) {
		return framework.MonClientV1.ServiceMonitors(ns).Get(prometheusName, metav1.GetOptions{})
	...
	for _, resource := range resourceDefinitions {
		if len(resource.Versions) > resource.MaxExpectedChanges || len(resource.Versions) < 1 {
			...
			t.Fatalf("expected resource %v to be created/updated %v times, but saw %v instead", resource.Name, resource.MaxExpectedChanges, len(resource.Versions))
		}
	}
}
...
func testPromDiscoverTargetPort(t *testing.T) {
	...
	if _, err := framework.MonClientV1.ServiceMonitors(ns).Create(&monitoringv1.ServiceMonitor{ObjectMeta: metav1.ObjectMeta{Name: prometheusName, Labels: map[string]string{"group": group}}, Spec: monitoringv1.ServiceMonitorSpec{Selector: metav1.LabelSelector{MatchLabels: map[string]string{"group": group}}, Endpoints: []monitoringv1.Endpoint{{TargetPort: &targetPort, Interval: "30s"}}}}); err != nil {
		t.Fatal("Creating ServiceMonitor failed: ", err)
	}
	p := framework.MakeBasicPrometheus(ns, prometheusName, group, 1)
	if _, err := framework.CreatePrometheusAndWaitUntilReady(ns, p); err != nil {
...
func testPromOpMatchPromAndServMonInDiffNSs(t *testing.T) {
	...
	if err := testFramework.AddLabelsToNamespace(framework.KubeClient, serviceMonitorNSName, map[string]string{"team": "frontend"}); err != nil {
		t.Fatal(err)
	}
	group := "sample-app"
	...
	p.Spec.ServiceMonitorNamespaceSelector = &metav1.LabelSelector{MatchLabels: map[string]string{"team": "frontend"}}
...
func testThanos(t *testing.T) {
	...
	version := "v0.1.0"
	basicPrometheus.Spec.Thanos = &monitoringv1.ThanosSpec{Peers: &peerServiceDNS, Version: &version}
	basicPrometheus.Spec.PodMetadata = &metav1.ObjectMeta{CreationTimestamp: metav1.Now(), Labels: map[string]string{"thanos-peer": "true"}}
	replicas := int32(2)
	...
	querierArgs := []string{"query", "--log.level=debug", "--query.replica-label=prometheus_replica", fmt.Sprintf("--cluster.peers=%s", peerServiceDNS)}
...
func testPromGetBasicAuthSecret(t *testing.T) {
	...
	maptest["tc"] = ns
	prometheusCRD := framework.MakeBasicPrometheus(ns, name, name, 1)
	prometheusCRD.Spec.ServiceMonitorNamespaceSelector = &metav1.LabelSelector{MatchLabels: maptest}
	if _, err := framework.CreatePrometheusAndWaitUntilReady(ns, prometheusCRD); err != nil {
		t.Fatal(err)
	}
	...
	authSecret := &v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: name}, Data: map[string][]byte{"user": []byte("user"), "password": []byte("pass")}}
	if _, err := framework.KubeClient.CoreV1().Secrets(testNamespace).Create(authSecret); err != nil {
		t.Fatal(err)
	}
	svc := &v1.Service{ObjectMeta: metav1.ObjectMeta{Name: name, Labels: map[string]string{"group": name}}, Spec: v1.ServiceSpec{Type: v1.ServiceTypeLoadBalancer, Ports: []v1.ServicePort{v1.ServicePort{Name: "web", Port: 8080}}, Selector: map[string]string{"group": name}}}
	sm := framework.MakeBasicServiceMonitor(name)
	sm.Spec.Endpoints[0].BasicAuth = &monitoringv1.BasicAuth{Username: v1.SecretKeySelector{LocalObjectReference: v1.LocalObjectReference{Name: name}, Key: "user"}, Password: v1.SecretKeySelector{LocalObjectReference: v1.LocalObjectReference{Name: name}, Key: "password"}}
...
func testOperatorNSScope(t *testing.T) {
	...
	t.Run("SingleNS", func(t *testing.T) {
		...
		p.Spec.RuleNamespaceSelector = &metav1.LabelSelector{MatchLabels: prometheusNamespaceSelector}
		p.Spec.EvaluationInterval = "1s"
...
func assertExpectedTargets(targets []*testFramework.Target, expectedTargets []string) error {
	...
	if !reflect.DeepEqual(expectedTargets, existingTargets) {
		return fmt.Errorf("expected targets %q but got %q", strings.Join(expectedTargets, ","), strings.Join(existingTargets, ","))
	}
	return nil
}
