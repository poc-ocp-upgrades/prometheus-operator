...
func (f *Framework) AddAlertingToPrometheus(p *monitoringv1.Prometheus, ns, name string) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	p.Spec.Alerting = &monitoringv1.AlertingSpec{Alertmanagers: []monitoringv1.AlertmanagerEndpoints{{Namespace: ns, Name: fmt.Sprintf("alertmanager-%s", name), Port: intstr.FromString("web")}}}
}
...
func (f *Framework) MakePrometheusService(name, group string, serviceType v1.ServiceType) *v1.Service {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	service := &v1.Service{ObjectMeta: metav1.ObjectMeta{Name: fmt.Sprintf("prometheus-%s", name), Labels: map[string]string{"group": group}}, Spec: v1.ServiceSpec{Type: serviceType, Ports: []v1.ServicePort{{Name: "web", Port: 9090, TargetPort: intstr.FromString("web")}}, Selector: map[string]string{"prometheus": name}}}
	return service
}
...
func (f *Framework) DeletePrometheusAndWaitUntilGone(ns, name string) error {
	...
	if err := WaitForPodsReady(f.KubeClient, ns, f.DefaultTimeout, 0, prometheus.ListOptions(name)); err != nil {
		return errors.Wrap(err, fmt.Sprintf("waiting for Prometheus custom resource (%s) to vanish timed out", name))
	}
	return nil
}
func (f *Framework) WaitForPrometheusRunImageAndReady(ns string, p *monitoringv1.Prometheus) error {
	...
	return WaitForPodsReady(f.KubeClient, ns, f.DefaultTimeout, int(*p.Spec.Replicas), prometheus.ListOptions(p.Name))
}
...
func (f *Framework) CheckPrometheusFiringAlert(ns, svcName, alertName string) (bool, error) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	response, err := f.QueryPrometheusSVC(ns, svcName, "/api/v1/query", map[string]string{"query": fmt.Sprintf("ALERTS{alertname=\"%v\"}", alertName)})
	if err != nil {
		return false, err
	}
...
func (f *Framework) WaitForPrometheusFiringAlert(ns, svcName, alertName string) error {
	...
	if err != nil {
		return errors.Errorf("waiting for alert '%v' to fire: %v: %v", alertName, err.Error(), loopError.Error())
	}
	return nil
}
