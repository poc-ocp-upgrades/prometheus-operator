...
func New(c prometheusoperator.Config, logger log.Logger) (*Operator, error) {
	...
	o.ssetInf = cache.NewSharedIndexInformer(listwatch.MultiNamespaceListerWatcher(o.config.Namespaces, func(namespace string) cache.ListerWatcher {
...
func (c *Operator) waitForCacheSync(stopc <-chan struct{}) error {
	...
	}{{"Alertmanager", c.alrtInf}, {"StatefulSet", c.ssetInf}}
...
func (c *Operator) createCRDs() error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	crds := []*extensionsobj.CustomResourceDefinition{k8sutil.NewCustomResourceDefinition(c.config.CrdKinds.Alertmanager, c.config.CrdGroup, c.config.Labels.LabelsMap, c.config.EnableValidation)}
	...
	}{{"Alertmanager", listwatch.MultiNamespaceListerWatcher(c.config.Namespaces, func(namespace string) cache.ListerWatcher {
		return &cache.ListWatch{ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
