...
var (
	defaultTestConfig = Config{ConfigReloaderImage: "quay.io/coreos/configmap-reload:latest", AlertmanagerDefaultBaseImage: "quay.io/prometheus/alertmanager"}
)
...
func TestStatefulSetPVC(t *testing.T) {
	...
	sset, err := makeStatefulSet(&monitoringv1.Alertmanager{ObjectMeta: metav1.ObjectMeta{Labels: labels, Annotations: annotations}, Spec: monitoringv1.AlertmanagerSpec{Storage: &monitoringv1.StorageSpec{VolumeClaimTemplate: pvc}}}, nil, defaultTestConfig)
	require.NoError(t, err)
...
func TestStatefulEmptyDir(t *testing.T) {
	...
	sset, err := makeStatefulSet(&monitoringv1.Alertmanager{ObjectMeta: metav1.ObjectMeta{Labels: labels, Annotations: annotations}, Spec: monitoringv1.AlertmanagerSpec{Storage: &monitoringv1.StorageSpec{EmptyDir: &emptyDir}}}, nil, defaultTestConfig)
	require.NoError(t, err)
...
func TestListenLocal(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	sset, err := makeStatefulSet(&monitoringv1.Alertmanager{Spec: monitoringv1.AlertmanagerSpec{ListenLocal: true}}, nil, defaultTestConfig)
	if err != nil {
...
func TestMakeStatefulSetSpecMeshClusterFlags(t *testing.T) {
	...
	}{{"v0.14.0", "mesh", "cluster"}, {"v0.15.3", "cluster", "mesh"}}
	for _, test := range tests {
		a := monitoringv1.Alertmanager{}
		a.Spec.Version = test.version
		replicas := int32(3)
...
func TestAdditionalSecretsMounted(t *testing.T) {
	...
	sset, err := makeStatefulSet(&monitoringv1.Alertmanager{ObjectMeta: metav1.ObjectMeta{}, Spec: monitoringv1.AlertmanagerSpec{Secrets: secrets}}, nil, defaultTestConfig)
	require.NoError(t, err)
	secret1Found := false
	secret2Found := false
...
func TestSHAAndTagAndVersion(t *testing.T) {
	...
	defer _logClusterCodePath()
	{
		sset, err := makeStatefulSet(&monitoringv1.Alertmanager{Spec: monitoringv1.AlertmanagerSpec{Tag: "my-unrelated-tag", Version: "v0.15.3"}}, nil, defaultTestConfig)
		if err != nil {
			t.Fatalf("Unexpected error while making StatefulSet: %v", err)
		}
	...
	{
		sset, err := makeStatefulSet(&monitoringv1.Alertmanager{Spec: monitoringv1.AlertmanagerSpec{SHA: "7384a79f4b4991bf8269e7452390249b7c70bcdd10509c8c1c6c6e30e32fb324", Tag: "my-unrelated-tag", Version: "v0.15.3"}}, nil, defaultTestConfig)
		if err != nil {
			t.Fatalf("Unexpected error while making StatefulSet: %v", err)
		}
		image := sset.Spec.Template.Spec.Containers[0].Image
	...
	}
	{
		...
		sset, err := makeStatefulSet(&monitoringv1.Alertmanager{Spec: monitoringv1.AlertmanagerSpec{SHA: "7384a79f4b4991bf8269e7452390249b7c70bcdd10509c8c1c6c6e30e32fb324", Tag: "my-unrelated-tag", Version: "v0.15.3", Image: &image}}, nil, defaultTestConfig)
		if err != nil {
			t.Fatalf("Unexpected error while making StatefulSet: %v", err)
		}
...
func TestRetention(t *testing.T) {
	...
	for _, test := range tests {
		sset, err := makeStatefulSet(&monitoringv1.Alertmanager{Spec: monitoringv1.AlertmanagerSpec{Retention: test.specRetention}}, nil, defaultTestConfig)
		if err != nil {
			t.Fatal(err)
		}
